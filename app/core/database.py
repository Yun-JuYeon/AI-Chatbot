...
DATABASE_URL = (
    f"postgresql+psycopg2://{pg_username}:{pg_password}@localhost:{pg_port}/{pg_db}"
)

engine = create_engine(
    DATABASE_URL, echo=False
)  # echo는 추가적인 로그를 자동으로 찍어줌(True 일 경우)
metadata = MetaData()
...
def upsert_chat(user_id: str, chat_id: str, messages: list[dict]):
    with engine.connect() as conn:
        ...
        existing_chat = conn.execute(
            chat_info.select().where(chat_info.c.chat_id == chat_id)
        ).fetchone()
        if existing_chat:
            update_query = (
                chat_info.update()
                .where(chat_info.c.chat_id == chat_id)
                .values(messages=messages_str, created_at=func.now())
            )
            ...
            insert_query = chat_info.insert().values(
                user_id=user_id, chat_id=chat_id, messages=messages_str
            )
...
def get_chats(user_id: str, chat_id: str):
    with engine.connect() as conn:
        select_query = chat_info.select().where(
            chat_info.c.user_id == user_id, chat_info.c.chat_id == chat_id
        )
...
def get_all_chats(user_id: str):
    with engine.connect() as conn:
        select_query = chat_info.select().where(chat_info.c.user_id == user_id)
        result = conn.execute(select_query)
...
if __name__ == "__main__":
    upsert_chat(
        ...
        "test_chat_id3",
        [
            {"role": "user", "parts": ["채팅테스트!"]},
            {"role": "assistant", "parts": ["안녕하세요, 무엇을 도와드릴까요?"]},
        ],
    )
